...
- **受付の開始・終了は `AUDITION_STATUS`（Renderダッシュボードで `open` / `closed` を設定）で切り替える。**
  `render.yaml` では `sync: false` にしてあるため、ダッシュボードの設定はデプロイでは上書きされない
  （2026年9月に `value: closed` で固定していたことがあり、デプロイのたびに受付が自動的に閉じてしまい、
  8月31日以降の応募が保存されない事故があった。今後 `render.yaml` に `AUDITION_STATUS` の `value:` を
  直接書かないこと）。募集を開始・終了するときは必ずRenderダッシュボードの環境変数で切り替える。
